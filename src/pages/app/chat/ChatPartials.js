...
import { findUpper, getNomeHospital } from "../../../utils/Utils";
import { ChatContext } from "./ChatContext";

export const MeChat = ({ item, chat }) => {
  return (
  ...
  return (
    <div className="chat is-you">
      <div className="chat-avatar">
          <UserAvatar image={chat.image} theme={chat.theme} text={findUpper(chat.name)}></UserAvatar>
      </div>
      <div className="chat-content">
        <div className="chat-bubbles">
          {item.chat.map((msg, idx) => {
            return (
              <div className="chat-bubble" key={idx}>
                <div className="chat-msg" dangerouslySetInnerHTML={{__html: msg}}></div>
              </div>
            );
          })}
        </div>
        <ul className="chat-meta">
          {/* <li>{chat.name}</li> */}
          <li>{getNomeHospital(params.hospital)}</li>
        </ul>
      </div>
    </div>
  );
